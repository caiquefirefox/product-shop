...
public sealed class PedidosController(IPedidoService pedidos, IUsuarioService usuarios) : ControllerBase
{
    ...
    public async Task<ActionResult<PagedResultDto<PedidoDetalheDto>>> Listar([FromQuery] PedidoListQuery query, CancellationToken ct)
    {
        ...
        var usuario = await usuarios.ObterOuCriarAsync(usuarioEmail, usuarioId, usuarioNome, ct);
    ...
    public async Task<ActionResult<PedidoResumoMensalDto>> ResumoMensal([FromQuery] PedidoResumoMensalQuery query, CancellationToken ct)
    {
        ...
        var (de, ate, usuarioIdFiltro, statusIdFiltro, empresaIdFiltro, competenciaAnoMesFiltro) = query?.Normalize(agora) ?? (inicioPadrao, fimPadrao, (Guid?)null, (int?)null, (Guid?)null, (int?)null);
        ...
        var resumo = await pedidos.ObterResumoAsync(de, ate, usuario.Id, isAdmin, usuarioIdFiltro, statusIdFiltro, empresaIdFiltro, competenciaAnoMesFiltro, ct);
        return Ok(resumo);
    }
